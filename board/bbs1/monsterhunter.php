...
<body>
    ...
    $query = "select count(*) from bbs1 where id='monsterhunter'";

    if ($_catagory == "title") {
        $query = "select count(*) from bbs1 where subject like '%$_search%' and id='monsterhunter'";
    } elseif ($_catagory == "name") {
        $query = "select count(*) from bbs1 where name like '%$_search%' and id='monsterhunter'";
    } elseif ($_catagory == "content") {
        $query = "select count(*) from bbs1 where story like '%$_search%' and id='monsterhunter'";
    }
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container">
            <a class="navbar-brand" href="./monsterhunter.php">몬스터헌터 갤러리</a>
    ...
    <div class="container">
        <div class="mx-auto; d-block;" style="width:500px; height:auto; margin:20px auto">
            <img class="img-fluid" src="./monsterhunter.jpeg" alt="몬스터헌터" style="max-height:700px">
        </div>
    </div>
    <!-- 이미지 끝 -->
    <br><br>
    ...
    <div class="container">
        <table class="table table-hover">
            ...
            <tbody>
                <?php
                $query = "select * from bbs1 where id='monsterhunter' order by hit desc limit $view_top"; // desc 내림차순 asc 오름차순
                $result = mysql_query($query, $connect);
                $cnt = 1;
    ...
    <div class="container">
        <table class="table table-hover">
            <caption style="caption-side:top">
                ...
                <?php } else {
                    if ($_catagory == 'title') { ?>
                        <h5 id="list2"><strong>자유게시판 : <?php echo $_search . " 검색결과(제목)"; ?></strong></h5>
                    <?php } elseif ($_catagory == 'name') { ?>
                        <h5 id="list2"><strong>자유게시판 : <?php echo $_search . " 검색결과(글쓴이)"; ?></strong></h5>
                    <?php } elseif ($_catagory == 'content') { ?>
                        <h5 id="list2"><strong>자유게시판 : <?php echo $_search . " 검색결과(내용)"; ?></strong></h5>
                    <?php } ?>
                <?php } ?>
            </caption>
            <thead>
            ...
            <tbody>
                <?php
                $query = "select * from bbs1 where id='monsterhunter' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                if ($_catagory == "title") {
                    $query = "select * from bbs1 where id='monsterhunter' and subject like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                } elseif ($_catagory == "name") {
                    $query = "select * from bbs1 where id='monsterhunter' and name like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                } elseif ($_catagory == "content") {
                    $query = "select * from bbs1 where id='monsterhunter' and story like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                }
    ...
    <div class="container">
        <button type="button" class="btn btn-primary float-right"><a style="color:white;" href="./write.php?id=monsterhunter&genre=game">글쓰기</a></button>
    ...
    <div class="container">
        <div class="row mx-auto">
            <form class="form-inline" style="margin:0 auto;" action="./monsterhunter.php<?php echo '#list2'; ?>" method="get">
