...
<div id="page_1">
    ...
    if($_page != 1) echo ("<a href=$PHP_SELF?_page=$before_group$href>[◀]</a> &nbsp;"); // 이전페이지 그룹출력 // PHP_SELF: 현재 자신의 페이지에서 페이지 이름을 리턴해주는 역할

    // 페이지 번호 출력
    for($e=$goto; $e < $last; $e++) { // 현재 페이지가 전체페이지보다 작으면 페이지를 증가
        if($e > $rr) break; // 총 나타낼 페이지 번호보다 크면 멈추고 다음을 실행
        if($e == $_page) echo ("<strong>$e</strong>"); // $e와 페이지 번호가 서로 같으면
        else echo("&nbsp; <a href=$PHP_SELF?_page=$e$href>[$e]</a>&nbsp;"); // $e와 $_page번호가 서로 같지 않으면...
    }
