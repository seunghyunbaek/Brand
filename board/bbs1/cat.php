...
<body>
    ...
    // 변경7 : id 경로 변경하기
    $query = "select count(*) from bbs1 where id='cat'";

    if ($_catagory == "title") {
        $query = "select count(*) from bbs1 where subject like '%$_search%' and id='cat'";
    } elseif ($_catagory == "name") {
        $query = "select count(*) from bbs1 where name like '%$_search%' and id='cat'";
    } elseif ($_catagory == "content") {
        $query = "select count(*) from bbs1 where story like '%$_search%' and id='cat'";
    }
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container">
            ...
            <a class="navbar-brand" href="./cat.php">야옹이 갤러리</a>
    ...
    <div class="container">
        <div class="mx-auto; d-block;" style="width:500px; height:auto; margin:20px auto">
            <!-- 변경3 : 갤러리 메인 이미지  -->
            <img class="img-fluid" src="./cat.jpeg" alt="애용" style="max-height:700px">
        </div>
    </div>
    <!-- 이미지 끝 -->
    <br><br>
    ...
    <div class="container">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                $query = "select * from bbs1 where id='cat' order by hit desc limit $view_top"; // desc 내림차순 asc 오름차순
                $result = mysql_query($query, $connect);
                $cnt = 1;
    ...
    <div class="container">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                $query = "select * from bbs1 where id='cat' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                if ($_catagory == "title") {
                    $query = "select * from bbs1 where id='cat' and subject like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                } elseif ($_catagory == "name") {
                    $query = "select * from bbs1 where id='cat' and name like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                } elseif ($_catagory == "content") {
                    $query = "select * from bbs1 where id='cat' and story like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                }
    ...
    <div class="container">
        <!-- 변경 6 : 글쓰기 경로 id변경하기  -->
        <button type="button" class="btn btn-primary float-right"><a style="color:white;" href="./write.php?id=cat&genre=healing">글쓰기</a></button>
    ...
    <div class="container">
        <div class="row mx-auto">
            <!-- action의 페이지경로 변경 -->
            <form class="form-inline" style="margin:0 auto;" action="./cat.php<?php echo '#list2'; ?>" method="get">
