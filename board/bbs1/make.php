...
<body>
    ...
    // 변경 where id=
    $query = "select count(*) from bbs1 where id='make'";

    if ($_catagory == "title") {
        $query = "select count(*) from bbs1 where subject like '%$_search%' and id='make'";
    } elseif ($_catagory == "name") {
        $query = "select count(*) from bbs1 where name like '%$_search%' and id='make'";
    } elseif ($_catagory == "content") {
        $query = "select count(*) from bbs1 where story like '%$_search%' and id='make'";
    }
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container">
        <!-- 변경 -->
            <a class="navbar-brand" href="./make.php">갤러리신청 갤러리</a>
    ...
    <div class="container">
        ...
        <!-- 변경 -->
            <img class="img-fluid" src="./make.png" alt="신청갤" style="max-height:700px">
        </div>
    </div>
    <!-- 이미지 끝 -->
    <br><br>
    ...
    <div class="container">
        <table class="table table-hover">
            ...
            <tbody>
                <?php
                // 변경 id=
                $query = "select * from bbs1 where id='make' order by hit desc limit $view_top"; // desc 내림차순 asc 오름차순
                $result = mysql_query($query, $connect);
                $cnt = 1;
    ...
    <div class="container">
        <table class="table table-hover">
            ...
            <tbody>
                <?php
                // 변경 id=
                $query = "select * from bbs1 where id='make' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                if ($_catagory == "title") {
                    $query = "select * from bbs1 where id='make' and subject like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                } elseif ($_catagory == "name") {
                    $query = "select * from bbs1 where id='make' and name like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                } elseif ($_catagory == "content") {
                    $query = "select * from bbs1 where id='make' and story like '%$_search%' order by no desc limit $page, $view_total"; // desc 내림차순 asc 오름차순
                }
    ...
    <div class="container">
    <!-- 변경 id= genre= -->
        <button type="button" class="btn btn-primary float-right"><a style="color:white;" href="./write.php?id=make&genre=adm">글쓰기</a></button>
    </div>
    ...
    <div class="container">
        ...
        <!-- 변경 -->
            <form class="form-inline" style="margin:0 auto;" action="./make.php<?php echo '#list2'; ?>" method="get">
